...
随着复杂模型（例如 GPT-5-Codex 系列）的能力增强，我们希望进一步探索：**能否把阅读—检索—推理—写作链路交给智能体自动完成，或至少让它主导大部分工作？** 这个仓库就是为了验证这一想法而搭建，目标是让智能体自己探索素材、沉淀洞见，并把结果汇编成稿。

为了验证迭代成果，我们做了一个简易实验，产出可在 [`results/wechat_group_summary.md`](results/wechat_group_summary.md) 查看。我觉得这个文档的质量是远远不如我们上一次实验手工生成的质量高的，但是考虑到这个方向潜力还是很大的，所以把代码和提示词分享出来，方便大家在这个基础上继续探索。
